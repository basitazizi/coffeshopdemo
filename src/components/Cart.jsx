...
export default function Cart() {
  ...
  const submit = () => {
    ...
    setOrderNum(num);
    const order = {
      orderNum: num, tableNum, name, notes,
      items: cart, subtotal, tax, total,
      time: new Date().toLocaleTimeString(), status: 'New Order',
    };
    const existing = JSON.parse(localStorage.getItem('kitchen_orders') || '[]');
    localStorage.setItem('kitchen_orders', JSON.stringify([order, ...existing]));
    clearCart();
    setStep('confirmed');
  };

  const reset = () => { setStep('cart'); setTableNum(''); setName(''); setNotes(''); setCartOpen(false); };
        ...
        {step === 'checkout' && (
          <>
            ...
            <div style={{ padding: '20px 28px', borderTop: '1px solid rgba(212,137,74,0.2)', display: 'flex', gap: '12px', background: 'var(--teal-dark)' }}>
              ...
              <button onClick={submit} disabled={!tableNum} style={{
                flex: 1, padding: '14px', border: 'none', borderRadius: '10px',
                background: tableNum ? 'var(--amber)' : 'rgba(212,137,74,0.2)',
                color: tableNum ? '#fff' : 'rgba(245,240,232,0.3)',
                fontFamily: 'Playfair Display, serif', fontSize: '1.05rem',
                cursor: tableNum ? 'pointer' : 'not-allowed', transition: 'all 0.2s',
